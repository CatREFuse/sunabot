...
| `docs/design/conversation-artifact-capabilities.md` | 会话文件、图片、单一 canonical Workbench 与 Codex 输入输出的统一产物句柄和能力上下文 | 调整附件获取、聊天媒体、Workbench 路由、Codex 产物或文件外发前读取 |
| `docs/design/webfetch.md` | WebFetch 的公开工具契约、静态与动态网页抓取、相关内容筛选、出站安全和实施验收计划 | 新增或修改 WebFetch、动态渲染服务、网页正文抽取或联网工具安全边界前读取 |
| `docs/architecture/project-structure-plan.md` | 目标项目结构、模块边界、组件通信、数据分层、Native Core 与 NapCat Docker 边界 | 调整目录、拆分模块、设计协议或修改运行打包前读取 |
...
| `docs/migrations/upgrade-0.2.0-to-0.3.0.md` | 0.2.0 到 0.3.0 的单一 Workbench 合并、恢复点、冲突门禁与回滚 | 既有 0.2.0 实例升级到 0.3.0 前必读 |
| `docs/migrations/upgrade-old-versions-to-current.md` | 0.1.0—0.3.0 老实例逐级升级到当前 revision 的总路径、单一 Workbench 资源验收与回滚 | 老实例跨多个版本升级或接入当前单一 Workbench 资源寻址前必读 |
| `docs/migrations/agent-workbenches.md` | 0.1.2 历史双 Workbench 迁移、验证与回滚 | 处理 0.1.0/0.1.1 历史实例时读取；当前运行合同以 0.3.0 单 Workbench 为准 |
...
| `docs/user-tests/current-system-tools.md` | 当前 Agent 工具目录的角色、平台与真实 Provider user test 计划 | 修改工具注册、会话权限、Native Bash 或工具验收矩阵时读取 |
| `docs/user-tests/soul-package-roundtrip.md` | 当前 Agent 人格提示词灵魂文件的导出、预览、冲突导入与往返 | 修改灵魂文件 schema、批量提示词事务、WebUI 或 CLI 入口时读取 |
| `docs/user-tests/offline-release-first-run.md` | Linux/WSL 自包含发行版、安装完整性、零下载启动与首次 Landing | 修改发行资产、安装脚本、首次启动或管理员初始化时读取 |
| `docs/user-tests/native-core-single-container-boundary.md` | Native Core 与每账号一个 NapCat 容器的唯一运行边界 | 修改 Core、NapCat、Bash、MCP、Skill 或 Renderer 运行形态时读取 |
| `docs/user-tests/webfetch-lightpanda-dynamic.md` | Linux/WSL Lightpanda 动态抓取、macOS unavailable 与静态回退 | 修改 WebFetch 动态 Renderer、平台能力或发行依赖时读取 |
...
| `docs/user-tests/workbench-image-docker-absolute-reference.md` | 0.2.0 Docker Workbench 历史兼容用例 | 维护 0.2.0 到 0.3.0 迁移或历史回归时读取 |
| `docs/user-tests/single-workbench-resource-addressing.md` | 当前 Agent 单一 Workbench 的表情、自拍参考图和知识资料寻址 | 修改 Workbench 资源管理与寻址时读取 |
...
| `docs/user-tests/user-private-attachment-docker-workbench.md` | 0.2.0 Docker Workbench 历史附件用例 | 维护 0.2.0 到 0.3.0 迁移或历史回归时读取 |
...
| 优先级 | 当前开放工作 |
| --- | --- |
| P1 | `DEPLOY-FIX-005` 真实 Linux/WSL Native Core + 多 NapCat Docker 的 amd64/arm64 安装、双 QQ 首次运行、账号定向外发、动态 WebFetch 和重启恢复验收 |
...
- Sunabot Core、Bash、MCP、Skill Script 与 WebFetch 在当前版本只以 Native 形态运行；NapCat 是唯一 Docker 运行组件，每个 QQ 使用独立容器。禁止重新引入 Core、Bash、MCP、Skill Script 或 WebFetch Docker 运行路径。
- 根目录 `./sunabot.sh` 是唯一人工运行入口，支持 `up|start|down|restart|status|logs|doctor|soul`；`up`、`start` 与 `restart` 使用同一套清空后启动流程。`SUNABOT_CORE_MODE` 与 `--core` 已退役，传入时必须明确拒绝。
- 启动与迁移必须由拥有仓库和 workspace 的非 root 用户执行；不得通过 UID 0 绕过 Core、NapCat、Bubblewrap 或 workspace 权限边界。
- 管理台只监听宿主回环 `127.0.0.1:8787`。OneBot v11 使用专用 `8788` 端口和强制 access token；Native Core 通过启动器配置的容器可达宿主网关接收 NapCat 连接，不得把 OneBot 端口直接发布到局域网或公网。
- 跨组件媒体默认使用 OneBot `base64://` 内联数据。业务代码、Core 与 NapCat 不能依赖共享绝对路径、相同挂载点或容器内文件路径；大文件能力必须新增明确、鉴权、可限流的传输契约。
- 每个 Agent 只有 `workspace/business/agents/<agentId>/workbench/` 一个 canonical Workbench。旧 `docker-workbench/` 只允许作为 0.2.0 到 0.3.0 的停服迁移输入；运行时、管理 API、管理台和新测试不得创建或寻址该目录。
- Linux/WSL 的 Native Bash、MCP、Skill Script 与 Lightpanda Renderer 使用随发行包提供的 Bubblewrap 隔离；macOS Bash 只对通过逐命令审批的管理员 QQ 私聊和已认证管理员 Web Chat 开放。macOS 动态 WebFetch 明确 unavailable，静态 WebFetch 继续可用。
- 平台差异只能放在 `apps/*` 组合根、`tooling/runtime/`、`deploy/` 或明确的平台 adapter 中，禁止在 services、领域模型和持久化模块中散布平台判断。
- 新功能涉及文件、进程、路径、附件、图片、工具、OneBot 或部署时，必须验证 Linux/WSL Native Core + NapCat Docker 的 amd64/arm64 组件边界；至少运行 `npm run runtime:contract`、相关单元测试、`npm run check` 与 `npm run build`。
- Linux/WSL 发行包必须内置 Node、生产 `node_modules`、Codex CLI、Lightpanda 与 Bubblewrap；安装阶段只允许准备摘要锁定的 NapCat 上游镜像，普通启动不得下载依赖。NapCat/QQ 的公开再分发授权尚未确认，因此不进入 Sunabot 发行归档。
- 平台专属依赖缺失时，启动器或 doctor 必须返回明确状态；不得静默切换运行模式，不得同时运行旧单容器和当前分离运行时，也不得改变数据库、配置或 workspace 的跨平台格式。
...
- 主对话前置数据必须写入 case 的 `input.fixture`，由 harness 在 raw OneBot ingress 前机械 seed 隔离工作记忆、长期记忆、用户画像、AIR 或 canonical Workbench 文件；不能只在 Markdown 段落中要求 fixture Agent 手工猜测或准备。静默、媒体等 case 使用 outbound kind 断言区分 `message`、`asset` 与 `poke`。
