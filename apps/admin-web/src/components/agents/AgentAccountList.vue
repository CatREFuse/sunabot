...
<template>
  <section class="mt-10 border-t border-line pt-6">
    <div class="flex items-center justify-between gap-4">
      <div>
        ...
        <h3 class="mt-2 text-xl font-medium text-display">接入账号</h3>
      </div>
      <button class="btn min-h-10 px-4" type="button" @click="createOpen = true">
    ...
    <div v-if="accounts.length" class="mt-5 border-t border-line">
      <div v-for="account in accounts" :key="account.id" class="flex min-h-20 items-center gap-4 border-b border-line py-3">
        ...
        <span class="font-mono text-[10px] uppercase" :class="account.connected ? 'text-success' : 'text-mute'">{{ account.connected ? "在线" : account.runtimeReady ? "离线" : "待启动" }}</span>
        <button class="btn min-h-9 px-3" type="button" :disabled="!account.runtimeReady" @click="openLogin(account)">{{ account.connected ? "账号" : account.runtimeReady ? "登录" : "重启后登录" }}</button>
    ...
    <div v-if="!accounts.length" class="empty-state min-h-48 py-12">
      <div><strong>还没有 QQ 账号</strong><p>新增 QQ 后，重启 Sunabot 即可登录</p></div>
    </div>
  </section>
  ...
  <DialogOverlay :open="createOpen" labelledby="create-account-title" @close="createOpen = false">
    <form class="w-full max-w-md border border-visible bg-panel p-6" @submit.prevent="create">
      <div class="flex items-center justify-between gap-4">
        <h2 id="create-account-title" class="text-xl font-medium text-display">新增 QQ</h2>
      ...
      <div class="mt-8 flex justify-end gap-3">
        ...
        <button class="btn btn-primary" type="submit" :disabled="busy || !draft.label.trim()">新增 QQ</button>
      </div>
    </form>
  </DialogOverlay>
