...
import type { ModelCallStatsPayload } from "../../types";
...
const memoryRows = computed(() => [
  { id: "working", label: "工作记忆", bucket: props.stats?.memory.kinds.working },
  { id: "long_term", label: "长期记忆", bucket: props.stats?.memory.kinds.long_term },
...
<template>
  <section class="border-y border-line" aria-label="模型调用统计">
    <header class="flex flex-wrap items-end justify-between gap-3 px-4 py-3 md:px-6">
      <div>
      ...
      <p v-else class="font-mono text-[10px] text-mute">
        <span v-if="messages != null" :title="exact(messages)">{{ metric(messages) }} 条消息 · </span><span :title="exact(stats?.total.requests)">{{ metric(stats?.total.requests) }} 次</span> · <span :title="exact(stats?.total.total)">{{ metric(stats?.total.total) }} Token</span>
      </p>
    </header>

    <div class="grid border-t border-line sm:grid-cols-2 xl:grid-cols-4">
      <div
        v-for="row in behaviorRows"
        :key="row.id"
        class="border-b border-line px-4 py-3 last:border-b-0 sm:[&:nth-last-child(-n+2)]:border-b-0 sm:[&:nth-child(odd)]:border-r xl:border-b-0 xl:border-r xl:last:border-r-0"
      >
    ...
    <div class="grid border-t border-line sm:grid-cols-3">
      <div
