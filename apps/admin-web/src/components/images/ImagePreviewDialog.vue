...
<template>
  <DialogOverlay :open="Boolean(image)" placement="full" backdrop="preview" :z-index="80" aria-label="图片预览" @close="emit('close')">
    <div v-if="image" class="mx-auto grid h-full min-h-0 w-full max-w-7xl grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden rounded-xl border border-white/20 bg-black text-white">
      <header class="flex items-center justify-between gap-4 border-b border-white/15 px-4 py-3">
        ...
        <button class="icon-btn border-white/30 text-white hover:border-white hover:text-white" type="button" aria-label="关闭预览" @click="emit('close')"><X :size="19" :stroke-width="1.5" /></button>
      </header>
      ...
      <footer class="flex flex-wrap items-center justify-between gap-3 border-t border-white/15 px-4 py-3">
        ...
        <div class="flex gap-2">
          <button class="btn border-white bg-white text-black hover:border-white hover:bg-white/85 hover:text-black" type="button" :disabled="downloading" @click="emit('download', image)"><Download :size="15" :stroke-width="1.5" />{{ downloading ? "下载中" : "下载" }}</button>
        </div>
      </footer>
    </div>
  </DialogOverlay>
