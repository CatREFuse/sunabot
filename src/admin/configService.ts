...
function validateCatalogEffort(model: string, effort: ReasoningEffort | undefined, field: string) {
...
function optionalReasoningEffort(value: unknown, field: string) {
...
function pathString(value: unknown, field: string, mustBeRelative: boolean) {
  const text = requiredString(value, field, { trim: true, min: 1, max: 2_048 });
  if (mustBeRelative && path.isAbsolute(text)) badRequest("CONFIG_INVALID", "必须使用相对 Agent workspace 的路径。", field);
  if (mustBeRelative) {
...
function optionalSecretArray(value: unknown, field: string) {
...
function optionalIntegerArray(value: unknown, field: string, maxExclusive: number) {
  if (value == null) return [];
  if (!Array.isArray(value) || value.length > 32) {
    badRequest("CONFIG_INVALID", "必须是最多 32 项的序号列表。", field);
  }
  if (value.length && maxExclusive === 0) badRequest("CONFIG_INVALID", "没有可删除的 Key。", field);
  return [...new Set(value.map((item, index) => integer(item, `${field}.${index}`, 0, Math.max(0, maxExclusive - 1))))];
}
