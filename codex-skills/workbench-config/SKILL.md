...
3. Select the writable root:
   - Native Bash: the cwd is the authoritative Native `workbench/`; managed resources may be updated there after approval.
   - Docker Bash: `/workbench` is the writable Docker task area; `$SUNABOT_NATIVE_WORKBENCH` or `/workbench/native-workbench` is the read-only authoritative projection.
...
5. Use a specialized tool only for the part Bash cannot obtain or authorize:
   ...
   - `import_chat_emoji` can normalize and publish current chat media when exposed, including an administrator group where Docker cannot write the Native projection.
