...
export const LEGACY_TONE_BUBBLE_COUNT_RULE =
...
export const TONE_BUBBLE_COUNT_GUIDANCE =
  "根据内容长度判断文字气泡的拆分方式：信息较短时，可以按短句拆分，每个文字气泡只放一个短句，推荐最多 3 个；信息较长时，每个文字气泡承载一个完整段落，一个段落能讲清楚就只用一个文字气泡。内容确实需要更多独立短句或段落、或用户明确要求时，可以生成 3 个以上的文字气泡。用户明确要求只用一个气泡时，使用一个文字气泡输出。";
