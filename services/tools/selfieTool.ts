...
export const SELFIE_DIRECT_DELIVERY_TOOL_INSTRUCTION =
  "The generated image is saved and sent by the system after this tool completes; do not call send_file for the result.";
  ...
  parameters: {
    ...
    properties: {
      ...
      referenceImagePaths: {
        ...
        items: { type: "string" },
        maxItems: 1,
        description: "At most one path of an existing image in the current authorized workbench, including a portable knowledge/... path. Pass either a path relative to the Workbench root or the exact absolute path returned by Bash. For a Markdown image link returned by knowledge_search, resolve the link from its source document, prefix knowledge/ exactly once, and verify that exact Workbench path before use. An explicit Workbench-relative path already starts at the Workbench root and must be used unchanged. Host absolute paths must remain under the current Agent workbench; sandbox absolute paths start at /workbench. Never pass a URL, media handle, Base64 value, guessed path, or a path outside those roots. Use null when no workbench image is needed."
      },
