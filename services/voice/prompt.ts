...
export const VOICE_TRIGGER_POLICY = [
  ...
  "send_voice_message 只传需要朗读的正文；合成语言和在线音色始终由当前 Voice Profile 的默认语言决定，不受主会话语言限制，也不要为了语音语言改写正文。没有可用在线音色或语音已关闭时不要调用。",
].join("\n");
...
export function voicePromptVariables(
  ...
  return {
    "conversation.voice.settings": {
      enabled:
        profile.enabled &&
        profile.provider.voices[profile.defaultLanguage] !== null,
      defaultLanguage: profile.defaultLanguage,
      languages: VOICE_LANGUAGES.map((code) => ({
        code,
        label: languageLabel(code),
        voiceReady: profile.provider.voices[code] !== null,
      })),
    },
