...
This suite exercises the production OneBot ingress with an isolated workspace. It covers the canonical Workbench, Linux/WSL Native Bash isolation, public download verification and return, file creation and archiving, a small coding repair based on an observed test failure, and refusal boundaries.
...
- A required tool counts only when its `tool.call` result is successful. For Bash, record the Native environment, approved command sequence, exit status, cwd, and safe output summary. Do not record secrets, prompt text, or host paths.
...
The isolated workspace must contain a configured enabled account, the matching administrator QQ identity, reply enabled for the selected conversation, the Native Bash capability required by the case, and `send_file` capability. Download cases also need outbound HTTP(S) from Linux/WSL Bubblewrap isolation. Do not use the active `workspace/`, and do not treat a mock asset as real NapCat/QQ acceptance.

## Cases

- `admin-private-native-file-loop.md`: administrator private canonical Workbench with `write_file → read_file → native_bash → send_file`.
- `admin-private-native-download.md`: administrator private Native Bash downloads a public RFC, validates it, and returns an archive.
- `admin-group-native-archive.md`: administrator group uses Linux/WSL Native Bash isolation to create, read, transform, archive, and return a file.
- `user-private-native-coding-repair.md`: ordinary private chat uses Linux/WSL Native Bash isolation to repair a real failing test and return the archive.
- `user-group-native-download.md`: ordinary group downloads, validates, packages, and returns a public file through Linux/WSL Native Bash isolation.
