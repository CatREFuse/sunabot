...
- [x] 当前生产代码通过 144 个测试文件、702 项单元/集成测试、23 项 E2E 和 6 项明暗主题视觉矩阵测试。
...
- [ ] **DATA-004A｜P1｜模型调用行为统计与群聊成本详情**
  - 请求日志按回答、编排器和记忆压缩记录模型调用；记忆压缩继续拆分工作记忆、长期记忆和用户画像。
  - 日志页展示全局调用次数与 Token；每个群聊详情展示消息数、Token 和模型调用分类。
  - 完成：分类总量无重复计数，按会话 ID 精确聚合，重试按实际模型请求计数，聚合接口和四视口界面测试通过。
