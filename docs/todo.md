...
v0.3.0 当前交付合同：Core、Bash、MCP、Skill Script、Codex 与 WebFetch 均为 Native；每账号 NapCat 是唯一 Docker 例外；所有会话使用单一 canonical `workbench/`。Linux amd64/arm64 发行包自包含 Node、production `node_modules`、Codex、Lightpanda 与 Bubblewrap，安装期只准备锁定 NapCat 上游镜像，普通启动零下载。当前发布阻断以 Linux Native Core + 多 NapCat Docker 与 WSL2 Native Core + 多 NapCat Docker 的真实验收为准。
...
- [x] NapCat 已收敛为每账号独立 Docker 组件；Native Core 共用专用 OneBot listener、token 和内联媒体契约。
...
- [x] 目标结构、模块协议、Native 运行模型、业务流程审计和完成标准已形成文档。
...
- [x] **RELEASE-FIX-001｜Linux 自包含发行与迁移资产闭环**
  - Linux amd64/arm64 归档携带 Native Core、管理台、生产依赖、Node、Codex、Lightpanda/源码/许可、Bubblewrap、迁移脚本、门禁模块与当前文档；发行入口要求干净且 revision 稳定的源码并重新生产构建。
  - 证据：runtime contract 与 release integrity 校验版本、平台、Node、source commit、文件集合、权限与 SHA-256，篡改依赖、锁文件和陈旧构建 fixture 失败关闭；NapCat/QQ 不进入发行资产，安装程序只准备 component lock 中摘要固定的上游镜像，启动固定 `pull never`。
...
- [x] **FLOW-FIX-004｜群聊主回复内部话题判断**
  - 原始 `message_32` 保持最近最多 32 条历史的时间顺序、正文与完整元数据；主回复模型在同一次推理中内部梳理并行话题、判断当前话题并完成指代消解，最终只输出正常回复。
  - 独立分类 Provider 请求、专用模型配置、动态索引变量、附加上下文、增量分类状态和新建状态表均已下线；旧数据库中的历史表作为未知附加表随备份恢复保留，不参与当前运行。
  - 验证证据维护在群聊内部话题判断专项用例，不沿用已下线实现的历史通过结果。
...
- [x] **ONBOARD-003｜P1｜新 QQ 运行时调和**
  ...
  - 证据：Native Core 的宿主账号 daemon、管理台单账号“运行”接口、daemon 缺失/超时持久状态、重复调和、目标账号隔离和注册库不可读零容器操作回归通过。
...
- [x] **ONBOARD-004｜P1｜空 workspace 首次运行 E2E**
  ...
  - 证据：Cookie+CSRF、Provider HTTP、OneBot WebSocket 入站到 QQ 回复受控 E2E，以及 6 个边界各 resume/rollback 的 12 次真实子进程 SIGKILL 通过；真实 Linux/WSL Native Core + NapCat Docker 双 QQ 验收继续由 `DEPLOY-FIX-005` 跟踪。
...
- [x] **ONBOARD-005｜P2｜只读 CLI 不安装依赖**
  - help 成功退出；`status|doctor|logs|down` 零依赖安装。发行版 `bootstrap` 只校验随包依赖，`up|start|restart` 零组件下载、零容器构建和零镜像拉取。
  - 证据：help、`up`、`start`、`restart`、`--dev`、已删除 Core 模式参数的稳定拒绝和缺依赖只读命令回归通过。

## 2026-07-16 生产隔离并行功能队列

以下段落保留 2026-07-16 各独立 worktree 的功能与证据记录；v0.3.0 的当前运行、目录与验收边界以上方交付合同和 `docs/specs/index.md` 为准。

- [ ] **TOOL-FIX-001｜P1｜管理员 `system_config` 工具**
  - 管理员 QQ 私聊可以查询脱敏设置/状态并调整当前 Agent 的自动回复、ambient 群聊编排、Tavily 搜索，以及已存在群聊的回复与编排开关；已认证 Web Chat 只允许查询。Bash 平台路由和隔离能力不属于 `system_config` schema。
  ...
  - 风险：与 reply/runtime、ToolRegistry、Provider executor 和配置 schema 共享入口；统一集成前不得直接落入生产 checkout。当前搜索实现只有 Tavily，不能把未实现的多搜索引擎描述为已交付；`get_settings` 仍只返回最多 100 条群摘要，超过摘要范围的已知群必须通过 query-only `list_groups` 稳定分页选择。
...
- [ ] **MEDIA-FIX-002｜P1｜`send_file` 与默认禁用语音发送**
  - `send_file` 向允许回复的真实 QQ 私聊与群聊提供 capability，目标始终冻结为当前会话；管理员私聊、普通私聊和群聊都只发送当前 Agent canonical `workbench/` 中的文件。伪造 Function Call、直接 queue、伪造 durable outbox、sender 已关闭和根身份漂移均 fail closed。图片使用消息段，普通文件使用目标会话对应的上传 action，不能接受任意 QQ、群号或宿主绝对路径。
  ...
  - 验收：覆盖全部允许回复会话的 canonical Workbench、当前目标冻结、伪造调用零读取/零 outbox/零远端发送，以及图片/普通文件/语音协议、`account_id` 定向、越界、绝对路径、符号链接、非常规文件、大小/类型限制、排队后替换、读取失败、离线重试和未声明语音调用拒绝。
...
- [ ] **BASH-FIX-001｜P0｜分会话 Bash 审计、确认与强隔离**
  - 公开工具只有 `native_bash`。Linux/WSL 的真实 OneBot 私聊与群聊使用 Native Bubblewrap：管理员私聊为 `admin` access mode，其他会话为 `isolated`；已认证管理员 Web Chat 使用 `admin`。macOS 只向管理员 QQ 私聊和管理员 Web Chat 提供受审批的宿主 `/bin/bash`，其他会话不暴露 Bash。
  - 所有 access mode 只写当前 Agent canonical `workbench/`，Skill 与 MCP 配置只读；不存在容器 backend、Docker socket 或第二 Workbench。Bash 可与正文和其他工具组合，每条命令仍经过 10 秒硬期限的独立模型审计、确定性策略和一次性会话审批票据。
  - Native runner 固定 30 秒命令期限、TERM→KILL、PID/内存/CPU/文件大小/输出预算、并发上限、进程组清理和失败熔断。Bubblewrap、namespace 或资源限制 probe 失败时能力不可用，不回退普通宿主 Bash。
  - 验收覆盖固定会话路由、模型审计失败与超时、永久拒绝、一次性确认重放、路径/符号链接/挂载/子进程/环境逃逸、并发与预算、熔断恢复、残留进程组、launcher 超时、runtime contract、类型、架构和构建。
  - 未完成：Linux/WSL Native Bubblewrap 实机、macOS 管理员宿主 Bash、Core 强杀后的残留恢复和真实 QQ 全链路证据。
...
- [ ] **AGENT-CAP-001｜P1｜Agent 级 Skill 与 MCP 安装、迁移和渐进披露**
  ...
  - W2 Provider/runtime、管理员 API、MCP Host/transport/OAuth、Skill 激活/迁移与沙箱投影的历史集成证据由 clean commit `7a7c6e3d201e0bf81b9f66c8d1664ff3f6b0419b` 和 WebUI commit `47922e8c358a58c14354ef0b5b5b0bec07e20874` 保留。v0.3.0 当前收口要求 stdio MCP、Skill Script 和投影均使用 Native Bubblewrap，并补齐真实 OAuth/HTTP MCP、Linux/WSL Native 运行态与最终隔离冒烟。
  ...
  - 安装阶段可使用不含生产凭据的独立受控 downloader/validator 联网，依赖必须锁版本、hash、许可证与来源并产出 digest-pinned 不可变包，随后进入 Agent 扩展层；runtime Skill 继续 `network none`，Native Bash 只允许经独立审计的任务相关获取，并拒绝 `npx`、`uvx`、`pip`、`npm` 等运行时安装。
  ...
  - stdio 进程由 Native Bubblewrap、进程组、`--die-with-parent` 与 watchdog 清理负责，不能退化为 SDK 只终止 launcher PID 的默认 close。Streamable HTTP 使用自定义受控 fetch，逐跳验证 HTTPS/localhost、redirect 与 DNS，拒绝私网、link-local、metadata、代理环境和超限/超时响应；静态或环境 header 均不得覆盖 Authorization、`Mcp-Session-Id`、`MCP-Protocol-Version`、Host、Cookie 等保留或凭据头。正常关闭有 session 时先有界 `terminateSession()`，再 `client.close()`；405 可作为不支持 DELETE 处理。
  - hardened stdio 使用 SDK 最小 `Transport` 接口复用 Native launcher、进程组、watchdog 与资源限制，不依赖 SDK `StdioClientTransport` 的宿主 env/cwd/stderr 默认。禁用、迁移或关闭 Agent 时必须按 `agentId + serverId` 有界销毁 client、session、进程与全部缓存。
  ...
  - 本地 server 仅使用 stdio，停止顺序为关闭 stdin、`SIGTERM`、超时后 `SIGKILL`。远端使用 Streamable HTTP，除 localhost 外强制 HTTPS，并验证 Origin、DNS rebinding、认证、协议版本与 session 生命周期；OAuth 使用 PKCE 和 resource audience binding，禁止 token passthrough。远端 MCP 由 Core 的独立受控 HTTP client 执行，不能通过放宽 Native Bash 网络实现。
  ...
  - MCP 负例还要覆盖：未协商 capability 的列举/调用、多页刷新第二页失败、重复/循环 cursor、页数/总字节超限、第一页 output schema 在完整刷新后仍校验、progress 洪泛不能越过总超时、abort 后迟到响应丢弃且进程组有界清理、未来协议或 experimental capability fail closed，以及 Agent/server 生命周期结束后零残留 client/session/cache。
  ...
  - 组合风险单独判定：`read_file`/`native_bash` 与 remote MCP/网络会形成数据外发链，不能因为每项能力分别获准就自动组合放行；Provider response 与 batch-level policy 必须在任何前置副作用前统一拒绝或要求明确审批。

- [ ] **AGENT-CAP-002｜P1｜沙箱 Skill/MCP 投影与环境变量白名单**
  - Native Bubblewrap backend 按内容摘要冻结当前 Agent 已启用 Skill、脱敏后的 MCP 配置和明确声明且获批的环境变量名；Skill 与配置使用只读快照，canonical `workbench/` 是唯一可写根。配置只保存 env 名或 secret reference，值只按显式 allowlist 注入单个进程；禁止继承整份宿主 env、代理变量、生产 secrets、Docker socket、其他 Agent 目录、Agent workspace 或宿主绝对路径。
  ...
  - `/run/sunabot/extensions/mcp.json` 只投影非秘密配置与 secret reference。MCP secret 绝不能进入通用 `native_bash`、Skill 脚本或整个沙箱环境；每个 stdio server 启动时由宿主从 secret store 解析该 server 的 allowlist，以 clearenv/`env -i` 只注入该进程，server A 看不到 server B 凭据。远端 token 只在 Core 受控 HTTP transport 内组装；切换 Agent、禁用、迁移或删除时销毁旧投影、client、环境与 OAuth state。
  - Native Bash 允许经独立审计的任务相关出站获取，并保持无 Docker socket、宿主代理 env 与本地文件外发；Skill 与 stdio MCP 默认无网。声明网络需求的 server 只可进入按域名 allowlist 的独立受控执行配置，禁止宽泛 `*` egress。
  - 验收：覆盖 Skill/MCP 新增、删除、版本切换后的新沙箱可见性，digest 固定快照、只读 Skill/配置、`workbench` 持久化、环境变量缺失/拒绝/脱敏与零泄漏、跨 Agent 读取、roots 仅 `file:///workbench`、无 socket/无 HOME/无隐式网络、进程销毁和 capability probe。
...
- [ ] **INTEGRATION-001｜P0｜统一集成、完整验证与最终冒烟**
  ...
  - 2026-07 历史集成 head `87fdbbe622360253a59436410bd0128bc4cd84dc` 保留 Bash audit/approval/epoch handle 与 Agent extensions 的冲突解析证据。v0.3.0 需要对 Native Bash、Native Skill/MCP、单一 Workbench 和 NapCat 唯一 Compose 模板重新执行当前 revision 的定向测试、runtime contract、构建、release integrity 与 diff-check；旧分支数量不能替代当前证据。
  - 快速隔离功能验收已在 head `de04049faedb0b6a8c39c577f5220d4a748b7d99` 完成：业务矩阵 17 files / 420 tests、runtime smoke 85 tests、WebUI E2E 36 tests 与 `127.0.0.1:28787` 生产构建冷启动 API smoke 全部通过；专用 workspace 为 `/Users/tanshow/Developer/sunabot-dev-workspaces/final-acceptance-20260717`，OneBot 测试端口 `28878`，MCP stdio 强制 disabled，未复制生产配置、凭据、QQ 登录态、Agent workspace 或 SQLite，结束后两个测试端口均已释放。真实 QQ、OAuth Provider、远端 MCP、Linux/WSL Native Bubblewrap 与 NapCat Docker 仍需按当前外部环境边界验收，不能用 fixture 结果替代。
  ...
  - 2026-07-17 生产切换前已停服并创建、验证静态恢复点 `/Users/tanshow/Developer/sunabot/workspace/backups/sqlite-recovery/sqlite-recovery-20260717T084319551Z-0a5d67d5`，覆盖 3 个 Agent 的 6 个 SQLite 数据库；随后以 clean release worktree 部署 WebUI head，并用 hotfix commit `cf3e76c669c81b4972de9d9cf0558e397370f7f2` 补齐 `/extensions` SPA 深链。该次现场验证的 Node、端口、管理 API、OneBot health、历史记忆积压和三账号连接差异均作为历史证据保留；v0.3.0 仍需重新验证 OAuth/HTTP MCP、Linux/WSL Native Bubblewrap、锁定 NapCat 镜像和多账号全链路。
  ...
  - 手工：在隔离 Linux Native Core + 多 NapCat Docker 与 WSL2 Native Core + 多 NapCat Docker 环境验证发行安装、空 workspace Landing、管理员/Provider、双 QQ 登录、私聊/群聊/@/引用、文字/图片/文件、账号定向、灵魂文件往返、动态 WebFetch、启动零下载、重启与冷启动恢复、SQLite/queue/Agent 隔离、OneBot token/连接 owner 和无旧新 runtime split-brain。
  - 完整 launcher 固定使用 `8787/8788/8790/6099+`，不得与同机生产实例并行启动；真实 Native Core + NapCat Docker 验收必须在独立 QA 主机/VM 执行，或进入最终停服切换窗口后执行。现有 `smoke:runtime` 的 `127.0.0.1:18878` 回连只作为 fake NapCat 证据，标准 bridge-mode NapCat 容器不能把该宿主回环地址当真实回连证据。
  ...
  - 2026-07-17 生产证据记录了三个 NapCat 容器只有两条已建立 OneBot 连接的差异；容器 `healthy` 或 CLI `connected=unknown` 不能替代账号逐一真实在线和定向外发证据。
  - 任一数据完整性、重复实例、账号串发、路径越界、媒体错误、权限绕过、恢复失败或生产边界被触碰都阻断部署并按对应功能 commit 回滚。

- [ ] **DEPLOY-FIX-005｜P1｜双环境双 QQ 迁移与上线验收**
  - 当前开放环境固定为 Linux Native Core + 多 NapCat Docker 与 WSL2 Native Core + 多 NapCat Docker；两端都由拥有安装目录与 workspace 的非 root 用户通过根 `./sunabot.sh` 操作。已有实例完成停服、恢复点、逐级迁移、启动和回滚演练；fresh install 只初始化当前结构。禁止旧 Core 容器、旧单 Agent 与新运行时并行。
  ...
  - 两端分别验证 amd64/arm64 发行归档、安装期锁定 NapCat 镜像、首次 Landing、启动零下载、双 QQ 私聊、群聊、@、引用、文字、图片、文件、Base64 媒体、账号定向外发、异步 callback、outbox、灵魂文件往返、Lightpanda 动态 WebFetch、Agent/SQLite/queue 隔离、重启与冷启动恢复；WSL 需要分别覆盖 Docker Desktop 的 `127.0.0.1` + `host.docker.internal` 转发和发行版内 Docker Engine 的本机 bridge gateway，两个路径均执行容器内 OneBot 探针且不监听 LAN；同时核对 OneBot token、连接 owner、account runtime daemon 单实例和全部旧进程退出。
  ...
  - 2026-07-19 的 macOS Native Core + NapCat Docker 启动证据保留为历史平台记录，不能关闭当前 Linux/WSL 发行、Bubblewrap、双 QQ、零下载、冷启动和回滚验收。
...
- [x] **WEBUI-FIX-010｜模型调用统计与群聊详情**
  - 日志按回答、编排器、历史记忆调用和其他统计调用次数与 Token；`add_workmemory` 与 `add_user_profile` 属于主回复工具循环，不重复统计为独立记忆模型调用。
...
- [x] **GATE-001｜P0｜统一 Node 与 CI 运行时**（AUD-015、AUD-025）
  - 固定经过验证的 Node 24 小版本；`package.json`、CI、Native launcher 和 Linux 发行归档使用同一版本。
  - 完成：CI `npm run verify` 全绿，源码、launcher 与 amd64/arm64 release 的 `node --version` 一致，升级流程有回归清单。
  - 证据：`.node-version`、`.nvmrc`、package/lock、GitHub Actions、runtime contract、component lock 和 release 统一为 Node `24.18.0`；版本漂移 fixture 与 runtime contract 通过。
...
- [ ] **GATE-002｜P0｜runtime doctor 与唯一实例门禁**（AUD-024）
  ...
  - 启动/升级前发现 WSL/宿主重复进程、旧 Core 容器或多个 launcher 同时拥有 workspace 时必须失败退出，不自动抢占数据。
...
- [x] **GATE-006｜P1｜Native Bash 强隔离**（AUD-003）
  - Linux/WSL 使用随包 Bubblewrap、进程组和资源限制，不能只依赖字符串规则；macOS 只向管理员私聊开放逐命令审批的宿主 Bash。
  - 完成：Linux/WSL Bash 只能写当前 Agent canonical Workbench，路径、符号链接、挂载和子进程绕过测试通过；macOS 非管理员与群聊无宿主执行能力。
  - 证据：`services/tools/bashSandbox.ts` 固定只读宿主根与唯一可写 Workbench，子进程继承 mount/PID 隔离并丢弃 capability；缺失 Bubblewrap 或 namespace 能力时 fail closed，无容器回退。
...
- [ ] **CONTRACT-004｜P1｜单一 ToolRegistry**（AUD-017）
  ...
  - 删除 `tools.ts` 与 Provider 内的双重名字来源，统一 `native_bash` 等真实名称。
...
- [x] **DATA-004A｜P1｜模型调用行为统计与群聊成本详情**
  - 请求日志按回答、编排器、历史记忆调用和其他记录模型调用；兼容旧 `working`、`long_term` 与 `user_profile` 统计行，当前两个记忆工具随主回复计数且不重复建立记忆调用。
...
- [ ] **STRUCTURE-002｜P1｜消除 cwd 路径漂移**（AUD-024）
  ...
  - 完成：从任意 cwd 启动 Native Core、NapCat Docker 编排和脚本均指向同一 workspace；生产不再 cwd fallback。
...
- [ ] **STRUCTURE-004｜P2｜API/Web 依赖闭包分离**（AUD-027）
  - Web 构建依赖不进入 API production closure；release artifact 明确 production `node_modules` 和 Web 静态文件。
  - 完成：Native amd64/arm64 artifact 使用同一 production closure，启动不执行 npm install。

## M5：Native Core 与 NapCat Docker 交付

- [ ] **RUNTIME-001｜P1｜唯一 runtime contract 与组件锁**（AUD-022、AUD-027）
  - 定义管理台回环端口、OneBot 专用端口、NapCat Compose、宿主网关、media、secret、Native 启停、健康和 capability；锁 Node/NapCat/Codex/Lightpanda/Bubblewrap/officeparser 的版本、digest、checksum、license 和 architecture。
  - 完成：contract schema、component lock、SBOM、许可证和升级 smoke gate 入库。
  - 阶段证据：runtime contract、schema、component lock、Node/NapCat/Codex/officeparser/bubblewrap 版本与多架构信息已入库并通过静态门禁；NapCat/QQ 公开再分发授权尚未确认，component lock 保持 `pending-review`，SBOM 产物、其余许可证复核和升级 smoke 仍待完成。

- [ ] **RUNTIME-002｜P1｜Native Core 入口**（AUD-022）
  - Core 使用同一 artifact 和 workspace；NapCat 始终由独立 Docker 服务运行，Native 入口不能安装或启动 NapCat 宿主进程，只能调和每账号容器。
  - 完成：干净 Linux amd64/arm64 与 WSL 上的启动、停止、升级、冷启动、扫码、文字和 `base64://` 图片通过。
  - 阶段证据：旧 Native NapCat 启动脚本和 systemd unit 已删除，Native Core 只通过统一 launcher 管理独立 NapCat Docker；macOS 源码形态本机检查通过，干净 Linux/WSL 实机消息矩阵仍待完成。

- [ ] **RUNTIME-003｜P1｜Core/NapCat 分离交付与统一 launcher**（AUD-022）
  - Core 只以 Native artifact 运行；NapCat 使用摘要锁定的独立上游镜像，每账号一个 Compose project。公开再分发授权尚未确认，release 不内置 NapCat/QQ 镜像。
  - 根 `./sunabot.sh` 统一 `up|start|down|restart|status|logs|doctor|bootstrap|soul`；`up|start|restart` 使用同一套清空后启动流程。首次 `up` 包含管理员名称与密码 Landing。
  - 完成：Linux/WSL Native Core 通过宿主网关连接 NapCat；SIGTERM、冷启动、首次登录、真实文图消息、启动零下载和旧实例门禁通过。
  - 阶段证据：统一 launcher、多账号 NapCat 编排、专用 OneBot 端口、release 入口、账号运行时调和和 readiness 已实现；真实 Linux/WSL 多账号 smoke 仍待完成。
...
- [ ] **RUNTIME-005｜P1｜Native 平台 capability 合同**（AUD-022、AUD-025）
  - 对 websearch、图片、自拍、Codex、Bash、MCP、Skill、WebFetch、Office 正文解析、OneBot 文件和管理台建立同一 capability test。
  - 完成：同一 release/version/workspace fixture 在 Linux amd64、Linux arm64 与 WSL Native Core 通过；macOS 源码形态按合同明确动态 WebFetch unavailable。跨组件消息不包含共享绝对路径。
...
- [ ] **ROLLOUT-004｜P1｜规范与交付收口**
  ...
  - 完成：`npm run verify`、architecture、runtime/release contract、performance、Linux/WSL Native + NapCat Docker gates 全绿，逐项审计本文所有 TODO 后再标记目标完成。
