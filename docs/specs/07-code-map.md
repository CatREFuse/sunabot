...
| 表情提示词变量、标记规划、Tone 保护、内容 segment 与 OneBot 媒体门禁 | `services/agent/promptDefaults.ts`, `services/agent/promptWorkspace.ts`, `src/runtime/lifecycle.ts`, `src/runtime/emojiReply.ts`, `src/runtime/reply.ts`, `src/runtime/delivery.ts`, `services/emojis/public.ts`, `services/delivery/outboundMedia.ts`, `adapters/onebot/onebotGateway.ts` |
