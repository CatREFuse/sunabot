...
- `mutationFingerprint` 使用 SHA-256 绑定 Agent ID、完整 conversation ID、当前管理员 user ID、action、规范化参数和是否关闭当前私聊门控；不包含回复正文、时间、worker、随机 call ID、attempt 或密钥明文。同一事件重试只能复用相同 fingerprint，不同 fingerprint 必须失败关闭。session queue 的 held 状态不通过极远 `availableAt` 模拟，claim 只选择已释放记录，并继续保持会话与投递分区 FIFO。
- 只有宿主确认“管理员私聊、纯文本、唯一 `system_config` trace、该 mutation 明确关闭当前私聊自动回复”后，才可在确认 outbox 写入 `deliverySemantics: "system_config_confirmation"`。decoder 只接受缺省值或该唯一字面量。投递时重新校验当前管理员、私聊范围、无 group ID、正文与工具 trace，仅跳过本次配置提交造成的 private auto-reply/ReplyGate epoch 失效；发送者许可、会话记录启用状态、account ID、gateway 连接、FIFO、重试、remote receipt 和 settle 仍按普通 outbox 执行。模型参数、普通正文、`assistant_text`、deferred、图片、其他工具和外部 API payload 不能获得该语义。
