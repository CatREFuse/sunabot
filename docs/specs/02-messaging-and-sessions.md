...
- `assistant_text` 允许 Agent 在工具循环中发送中间消息。中间消息必须在当前 turn 仍运行时通过 SQLite outbox 持久化并完成远端发送，随后才能继续执行下一项 inline 工具；不能在 Provider 工具循环结束后与最终正文批量投递。重试同一事件时按事件 ID 与中间消息序号去重。群聊只引用第一条中间消息，最终正文仍引用原始消息，后续中间消息不引用。
