...
- 可解析的 JSON 与小程序卡片必须从顶层及有界嵌套的 `meta`、`detail`、`news`、`contact`、群邀请等结构中按标题、群名、群号、说明和动作顺序提取可读字段，再作为同一条消息正文进入 Session 快照、会话记录和后续模型历史；群号接受显式群字段，以及已确认属于群卡片的动作链接中的群参数。相同可见值只保留一次，动作 URL、应用内部标识、token、ticket 和未选中的原始序列化字段不能进入模型历史。解析最多访问 5 层、256 个节点并保留 12 个可见字段，卡片摘要仍受 240 字符总上限约束。
