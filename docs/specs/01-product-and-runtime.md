...
Native Core 启动时必须通过独立 `deploy/docker/compose.bash.yml` 准备 `sunabot-bash` 强隔离镜像。macOS Native Core 的全部真实 QQ Bash 命令都在该无网络、无 Docker socket 的短生命周期容器中执行，宿主 `/bin/bash` 固定禁用；Linux/WSL Native 与 Docker Core 使用 bubblewrap 强隔离。每条命令仍必须先通过独立对抗审批，Web Chat 不获得 Bash。隔离环境准备失败只降低 Bash capability 并输出稳定原因码，不能回退到宿主 shell；Docker 镜像不启动常驻服务、不发布端口，Skill 与 MCP 配置只读挂载，业务写入只进入当前 Agent 的隔离 workbench。
