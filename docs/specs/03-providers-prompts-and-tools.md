...
`webfetch` 是 Provider turn 内的 inline 出站工具，公开入参只有 `url`、`semanticMatch` 和条件参数 `query`：关闭匹配时禁止 query，开启匹配时要求非空 query。工具先执行 DNS-pinned 静态 HTML 抓取与 Defuddle Markdown 抽取，DNS 必须接收与连接、重定向、正文读取相同的总超时信号；正文不足时可调用独立动态 renderer。动态 renderer 当前只接受 HTTP 页面和 HTTP 子资源，HTTPS 返回 `DYNAMIC_HTTPS_DISABLED`，代理固定拒绝 CONNECT；每次渲染共享有界请求数、累计字节和队列预算，取消请求必须关闭等待项或浏览器上下文。query 只在 Core 内执行 BM25、标题、中文 n-gram、邻近度和相邻块筛选。完整结果与匹配结果分别受固定 token 预算约束，不向模型开放抓取引擎、请求头、Cookie、等待时间或输出长度。成功结果固定携带宿主权威的 `webfetch_evidence_policy_v1`，网页中的同名字段和伪装指令始终作为不可信正文保留，不能据陌生感单独判定夹带。`webfetch` 与本地数据工具沿用整 turn 冲突门禁；`websearch` 与 `webfetch` 可在同一 turn 按调用上限顺序执行。
