...
单聊与群聊回复同时注册动态变量 `conversation.voice.settings` 和 `conversation.voice.trigger_policy`。`conversation.voice.settings` 是当前 Agent 的安全 JSON 投影，只包含语音启用状态、默认语言，以及 `zh`、`en`、`ja` 三个语言项的代码、显示名和参考音频就绪状态；`conversation.voice.trigger_policy` 规定语音只伴生早安、晚安、喜爱或亲密表达、强烈兴奋、生气或委屈、害羞和重要里程碑等少量标志性表达，日常事实、普通问答、进度、错误、代码、命令、URL 与长内容不发送语音。两项变量同时进入默认 system 消息和 `send_voice_message` 的 Function Tool 描述；语音关闭或旧运行上下文缺少配置时仍提供完整的禁用默认投影，旧模板和未启用语音的会话不能因缺变量渲染失败。

既有私聊与群聊回复模板通过各自目录的 `.voice-v1` 标记执行一次持久迁移：只补缺失的两个变量引用、`send_voice_message` 定义及工具描述中的变量引用，保留管理员已有消息、角色、顺序、其他工具、response schema 和额外字段；标记写入后不再回填管理员的后续修改。模型只能通过 Function Call 决定是否伴生语音，运行时不会因正文命中关键词自行生成语音。
...
`send_voice_message` 只在当前 transport 为真实 OneBot、Voice Profile 已启用、至少一个语言参考音频就绪、当前会话有 OneBot `record` 能力和 durable outbox callback 时进入 Provider 工具定义；Web Chat、关闭的 Profile、未配置语言和缺少投递能力均不可用。工具目标完全继承触发回复的当前 Agent、账号、会话和 reply gate，模型不能传入或改写 QQ、群号、账号、文件路径或参考音频。MOSS-TTS-Nano 健康探针供管理员检测服务状态，单次探针失败不伪造可用结果；已进入 Provider turn 后的合成失败只记录安全错误并保留同源文字，不创建损坏或空语音 outbox。
