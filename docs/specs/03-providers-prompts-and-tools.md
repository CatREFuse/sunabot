...
单聊与群聊回复同时注册动态变量 `conversation.voice.settings` 和 `conversation.voice.trigger_policy`。`conversation.voice.settings` 是当前 Agent 的安全 JSON 投影，只包含语音启用状态、默认语言，以及 `zh`、`en`、`ja` 三个语言项的代码、显示名和参考音频就绪状态；`conversation.voice.trigger_policy` 规定语音只伴生早安、晚安、喜爱或亲密表达、强烈兴奋、生气或委屈、害羞和重要里程碑等少量标志性表达，日常事实、普通问答、进度、错误、代码、命令、URL 与长内容不发送语音。两项变量同时进入默认 system 消息和 `send_voice_message` 的 Function Tool 描述；模型只提供要朗读的正文，合成语言与参考音频始终取当前 Voice Profile 的默认语言，不受主会话语言限制，也不要求模型改写或翻译正文。语音关闭或旧运行上下文缺少配置时仍提供完整的禁用默认投影，旧模板和未启用语音的会话不能因缺变量渲染失败。
...
定时任务回调使用最终提示词 `scheduler.cron-callback` 和公共文件 `cron_callback.json`，沿用 Prompt Catalog、公共系统提示词继承、Agent 系统提示词覆盖、变量渲染和热更新机制。该提示词注册定时任务专属动态变量 `cron.payload`，值为有界、版本化的 JSON，包含任务 ID 与 revision、名称、上下文、cron/once 计划、本次 `scheduledFor`、`triggeredAt`、运行 ID 和全部回调目标；变量值作为不透明数据注入，运行时不得在模板渲染后强制插入、删除、去重或重排管理员消息。默认模板显式引用 `cron.payload`，不提供 Function Tool，使用文本输出；一次 occurrence 只执行一次 Provider 生成，投递层把同一正文分发到任务快照中的全部目标，并单独添加结构化 @。
...
`send_voice_message` 是普通回复 turn 的终止 companion 工具，严格参数对象只允许且必须包含 `text`，去除首尾空白后为 1—300 个字符；`language`、账号、目标和参考音频等额外字段全部拒绝。运行时只使用当前 Voice Profile 的 `defaultLanguage`，并在 Provider turn 接受前复核该语言的参考音频已就绪。一个 Provider turn 最多调用一次，并且只能使用以下四种封闭形态：可见 sibling assistant text 加末尾 `send_voice_message`；无 sibling text 的 `assistant_text` 加末尾 `send_voice_message`；无 sibling text 的单个 deferred tool 加末尾 `send_voice_message`，其 `dispatch_message` 作为可读来源；或上一轮唯一成功活动为一次已投递的 `assistant_text`，下一轮仅调用一次 `send_voice_message`。同一 response 内的后两项组合要求语音调用为第二项，跨轮形态不得夹入其他可见文字、工具或终止活动；四种形态都要求语音正文与来源正文在忽略已知表情标记并规范化空白后完全一致。语音调用后不能继续接受其他工具、第二份语音、`system_config` mutation 或后续模型轮次；任一形状、工具顺序、默认语言可用性、正文或 capability 不符时，在尚未发生的文本 callback、合成、任务和 outbox 前拒绝。
...
`workspace_bash` 默认只向无 `promptOverride` 的真实 OneBot 管理员私聊开放。入站必须同时绑定当前 Agent、非空账号、有效消息与 Bot 身份、管理员 user/sender；普通私聊、Web Chat、缺失或不匹配身份和默认群聊均不获得工具定义。管理员私聊使用当前 Agent 的 `adminPrivateBackend`；未来显式启用群聊还必须保持 `adminOnly=true`、`allowGroup=true`，并固定使用 Docker `restricted` 模式。API 工具目录只接收实时 capability 标记，策略允许的管理员 QQ 私聊或群聊范围必须独立于 backend 健康状态返回，不能因隔离探针失败退化为“当前没有可用会话”；运行异常另以 `runtime` 状态、当前私聊 backend，以及安全原因码 `BASH_AUDIT_UNAVAILABLE`、`BASH_WORKBENCH_UNAVAILABLE`、`BASH_NATIVE_ISOLATION_UNAVAILABLE` 或 `BASH_DOCKER_ISOLATION_UNAVAILABLE` 表达。API 不得取得 Provider 的 audit runner、审批上下文、宿主路径、沙箱诊断正文或可执行 Bash options。模型伪造未声明调用或不完整 options 时在沙箱启动前失败关闭。
...
`send_voice_message` 只在当前 transport 为真实 OneBot、Voice Profile 已启用、默认语言参考音频就绪、当前会话有 OneBot `record` 能力和 durable outbox callback 时进入 Provider 工具定义；Web Chat、关闭的 Profile、默认语言未配置和缺少投递能力均不可用。工具目标完全继承触发回复的当前 Agent、账号、会话和 reply gate，模型不能传入或改写语言、QQ、群号、账号、文件路径或参考音频。MOSS-TTS-Nano 健康探针供管理员检测服务状态，单次探针失败不伪造可用结果；已进入 Provider turn 后的合成失败只记录安全错误并保留同源文字，不创建损坏或空语音 outbox。
