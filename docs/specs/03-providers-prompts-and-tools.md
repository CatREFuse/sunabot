...
`workspace_bash` 向无 `promptOverride` 的真实 OneBot QQ 私聊和群聊开放。入站必须同时绑定当前 Agent、非空账号、有效消息与 Bot 身份以及一致的 user/sender；Web Chat、缺失或不匹配身份均不获得工具定义。全部允许会话固定使用 `docker/isolated` 路由并在每条命令前通过独立对抗审批 Agent；macOS Native Core 使用无网络、无 Docker socket、只读共享配置和资源上限的短生命周期容器，宿主 `/bin/bash` 固定不可调用，Linux/WSL Native 与 Docker Core 使用 bubblewrap 强隔离。API 工具目录只返回 Docker 隔离的配置、能力和固定会话范围；运行异常使用安全原因码 `BASH_AUDIT_UNAVAILABLE`、`BASH_WORKBENCH_UNAVAILABLE` 或 `BASH_DOCKER_ISOLATION_UNAVAILABLE`。API 不得取得 Provider 的 audit runner、审批上下文、宿主路径、沙箱诊断正文或可执行 Bash options。模型伪造未声明调用或不完整 options 时在执行环境启动前失败关闭。

Bash capability 必须把当前 Agent、隔离 backend、Docker workbench、Skill/MCP 只读共享目录、独立审批 Agent 可用性和执行探针作为一个原子结果；缺少任一依赖时 `available=false` 并返回与失败门禁对应的安全原因码，不能回退宿主 shell。Docker “已启动”表示真实只读根探针容器成功验证镜像依赖，不表示存在常驻共享容器。运行时每次配置替换都递增单调 `configEpoch`，单一 resolver 在同一不可变配置快照中冻结 epoch、真实入站身份、workbench、隔离模式、strict mode、审批上下文和独立 audit runner，再完成审计与执行探针；任一 await 后 epoch 改变只允许一次有界重探，仍变化则失败关闭。Provider 接收完整不可变 handle，并把同一 `isCurrent` 闭包传入 Bash runner；入口、路径准备、审计前后、审批 issue/consume、执行探针、invocation 构造及最终 spawn 前都必须复验。旧 handle 返回 `BASH_CONFIGURATION_STALE`，不得签发或消费审批、启动执行探针或执行命令。审批使用当前默认启用 Provider 的独立实例与 `bot.bash.auditModel`，请求只含审批 system/user 消息、`tools=[]` 和 strict JSON schema；票据绑定 Agent、账号、transport、完整会话、用户、可选群号和命令摘要，只能一次性消费。
...
单聊与群聊最终提示词以 `conversation.director.schedule` developer 变量接收当天已提交行程；值明确区分 active/unavailable，并把行程标为已提交事实。一次性迁移只为既有非空模板补齐变量与 canonical `call_director`，marker 后管理员删除或调整不再回填。`call_director` 是默认启用的 inline strict 工具，唯一参数 `request` 为 1—4,000 字的具体改期与世界内理由；工具只修改当前时间之后尚未开始的事项，已经开始或结束的事项必须逐字段保留，SQLite revision CAS 冲突要求角色重新请求。角色不得直接篡改行程，也不得为迎合用户无理由频繁改期。
...
输出使用 strict `memory_dream` JSON Schema，要求每条入选工作记忆与长期记忆恰好出现一次，梦境正文为 160—260 个 Unicode 字符且 `factuality=imagined`。长期记忆只能保留、重写、合并或提出归档建议，工作记忆只能保留、重写、合并、转存或丢弃；重写、合并、转存、丢弃和归档均携带置信度并由宿主二次校验，高风险动作低于 0.9 时退回保留。canonical fact 还必须通过宿主第一人称、回忆提示语、身份、高风险事实和来源 token/bigram 覆盖门禁，不能依靠模型自报置信度引入新事实。同事件与同因果链合并后保留原因、变化、结果、承诺、关系影响及仍有效的不确定性；共享事件键但因果键冲突时移除冲突键。梦境允许稳定种子控制的联想与轻微超现实，但不得补造现实经历，也不得成为事实纠错、归档或人格证据。人格建议只能是写入 `PREFERENCE.md` 或 `RELATION.md` 的一条 4—80 个 Unicode 字符温和倾向；宿主会确定性拒绝多句、模板标记、链接、绝对化承诺、核心身份或价值观修改、安全绕过、操控伤害和心理诊断等内容，不能只依赖模型自律。

Dream 的 Provider-facing schema 使用 Codex/OpenAI strict 子集，不发送 `uniqueItems`；来源 ID 与人格证据 ID 的唯一性继续由宿主解析器强制校验。既有公共或 Agent override `memory_dream.json` 通过 `.memory_dream.json.dream-schema-v2` 一次性迁移只移除该不兼容关键字，保留管理员消息、顺序和其他 response schema；渲染后的 Dream 请求在 Provider 调用前再次拒绝缺失 `response_format` 或仍含该关键字的管理员模板。Codex Responses 的确定性 HTTP 4xx 会携带不可重试语义返回 Dream 管线，不得被外层按暂时模型故障重新执行。
