...
`fact` 正文不能使用列表、字段标签、分类标题、来源说明、压缩说明或 `事实：`、`情绪：`、`认知：`、`相关用户：` 等模板化前缀。所有记忆正文只使用称呼作为人物语义标识；每个相关用户都必须在自然叙述中以“称呼（QQ 号）”出现，称呼与 QQ 号同时存在，涉及多人时逐一写全。用户画像以 `userId` 保存 QQ，以 `userName` 保存仅用于关联消息记录的当前显示名，以 `addressNames` 保存去重后的称呼数组；`userName` 不能进入正文充当人物标识。旧 `addressName`、`address_name` 与 `salutation` 在读取或画像合并时归一化为 `addressNames`，新记录只持久化数组。严格 JSON envelope、稳定 ID、来源边界、时间字段、事件键、来源工作记忆 ID 和事务语义保持不变。

用户画像 Provider 必须逐条检查本批 `messages`，把发送者名称、消息正文中真实出现且可指向该 QQ 的叫法及明确的“以后叫我……”表达，与 `previousProfiles.addressNames` 合并为完整称呼数组。运行时只接受当前批消息中实际出现、当前消息记录已观测、既有画像已保存或管理员配置明确允许的称呼；Provider 返回的 `userName` 只用于消息关联，不能成为称呼兜底。Provider 输出若把用户自述保留成角色记忆、使用回忆提示语、包含非法 QQ、使用未受信称呼或没有把称呼与 QQ 成对写入正文，该条事实必须失败关闭，不能进入工作记忆、长期记忆或用户画像；同一 QQ 在正文出现多次时每个标记都必须紧邻该 QQ 的受信称呼，额外 QQ 或真假称呼混用不能由一处正确配对掩盖。已有工作记忆非空时也不能借由非法输出清空旧快照。用户画像先提取并校验本批称呼，再把本批新增称呼提供给同批工作记忆和长期记忆使用，三类结果仍在一个 SQLite 事务中提交。当前实时长期记忆由 `memory.compress-in` 的受控晋升结果提交；`memory.compress-out` 保留为可编辑提示词目录和旧模板迁移兼容入口，不额外触发一轮实时 Provider 调用。

工作记忆与长期记忆的 strict 输出都必须包含 nullable `causalChainKey`。只有明确共享原因、转折与结果的同一因果主线才能复用 `causal:` 键；主题、时间或参与者相近不足以建立因果关系，无法确认时返回 `null`。运行时同时接受 camelCase 与 snake_case 输入，除 `null`/缺省外的空串、数组、越界或格式非法值会使整条事实失败关闭。`memory-perspective-v4` 一次性迁移会把已知 pre-causal v3 strict schema 升级为当前合同，保留管理员段落、其他消息、工具与 schema description；完全自定义的兼容模板只追加独立因果合同，第二次启动文件保持逐字节不变。

既有记忆按同一正文和压缩标准重整时，清洗结果必须拒绝“我记得”及同义中英文回忆提示语、三类来源中的用户自述式第一人称、前置或后置的用户第一人称直接引语、缺失或伪造为 QQ 号的称呼、正文中未成对出现的称呼与 QQ，以及没有覆盖全部证据时间的事件合并；多人事实中的每个 QQ 及同一 QQ 的每次出现都必须与证据提供的受信称呼精确对应。旧 `memory-perspective-v1` signed 提案或计划不满足称呼数组合同，不得继续安装，必须按新 schema 重新生成、签名并刷新。事件时间只能由证据确定性聚合，不能由提案任意改写或通过 `preserveFromBase` 覆盖。维护操作必须在 Core 完全停止后创建覆盖默认 Agent 与全部其他 Agent 业务库、队列库的可恢复备份并完成校验；参数化事务只在由该恢复点创建的完整 staging 副本中替换 `memory_records`，生产侧通过可重入 data 目录 journal 安装已全量验证的数据库集合，不能修改会话、队列、请求日志或其他业务表。重整后必须核对每个 Agent、每类来源的前后数量、样本、完整 row shape、`integrity_check`、queue 不变量和恢复能力。
...
当前召回使用内存 BM25，在工作记忆、长期记忆和用户画像中搜索。人物检索与召回提示只使用 QQ 和 `addressNames`，`userName` 不参与语义检索或提示身份展示。SQLite 负责持久化和有序读取，后续可在不改变调用接口的情况下增加 FTS 索引。
...
Dream 把睡眠研究中的记忆重放、显著信息筛选、旧记忆整合、要义抽取、联想式未来模拟和情绪调节映射为可审计的产品流程：近期与久远材料交错入选，重复事件按稳定事件或因果主线整理，低价值细节经高置信审查后压缩，未完成任务参与有界随机联想。突触降尺度只对应降低冗余与有保留期的归档，不对应无证据删除；记忆再巩固可能带来的失真风险通过来源覆盖、宿主事实校验、快照事务和人格 CAS 约束。
...
Dream 把工作记忆与长期记忆合并为一个候选池，以本次运行时间向前 `bot.memory.dreamRecentWindowHours` 小时为分界，使用运行 seed 对候选做无放回随机排序：近期最多抽取 `dreamRecentMemoryLimit` 条，更早记忆最多抽取 `dreamOlderMemoryLimit` 条；默认值分别为 48、12、12，窗口允许 1—720 小时，两桶分别允许 0—24 条且合计必须为 1—24 条。三项设置按 Agent 持久化并热更新，只影响随后新建的 Dream 运行；已经持久化输入的自动重试和人工恢复继续复用原 seed 与原批次。任一时间桶不足时只使用现有记录，不从另一桶补齐。最多 24 条入选记忆通过唯一的 `dream.payload` 变量一次性注入同一个 Provider 请求，在全批次中比较重复事件、相同因果与可合并信息；工作记忆和长期记忆仅因持久化动作不同而保留分组，不启动逐条模型调用。完成保留、重写、合并、转存或归档后分别写入 `dreamReviewedAt` 或 `lastReviewedAt`。读取阶段先把旧记录中的 `text/content/summary/memory` 正文、缺失或非法 ID 和旧称呼字段确定性归一为当前 v2 结构，原始快照摘要仍用于事务 CAS；管理台人工新增、置顶、受保护或明确要求记住的记录在保留、重写、合并、转存、丢弃与归档下都保持原记录逐字段不变。无关细节只有在模型给出以机器人第一人称为主体的规范正文、没有引用式他人第一人称或回忆提示语、来源中的称呼—QQ 配对完整保留、身份和高风险事实都有来源支持、来源 token 与 bigram 覆盖通过且置信度不低于 0.9 时才能重写、合并或转存。称呼与 QQ 必须按每条来源独立取证，禁止把不同来源的称呼和号码交叉组合；同一来源只有一个 QQ 时可绑定其多个受信称呼，多 QQ 来源必须在正文中逐一显式证明配对，证据不足整条失败关闭。任一宿主校验失败均逐条保留。模型返回后，宿主再次校验完整覆盖、来源 ID、事实性、归档策略与人格证据，再以工作记忆和长期记忆双快照摘要、运行租约及单个 SQLite 事务提交：压缩无关细节；工作记忆只有共享稳定 `eventKey` 或合法 `causalChainKey` 时合并，长期记忆还要求置信度不低于 0.9，并保留共同事件键；共享事件键下的冲突或非法因果键从合并结果移除，只有全部来源共有的合法因果键才能继承；转存值得保留的工作记忆，把入选的较早 Dream 按梦境日期分别转存为带 `imagined` 标记的长期记忆。关系不明或置信度不足时逐条保留，事务冲突时不做部分写入。

Provider 输出 schema 不使用 Codex Responses 不支持的 `uniqueItems`；工作记忆、长期记忆与人格证据中的重复、未知、遗漏或交叉复用 ID 全部由宿主的完整分区校验失败关闭。旧持久 Dream 模板只迁移不兼容 schema 关键字，不能覆盖管理员自定义提示词正文。
...
每次成功运行都新增一条 `memoryKind=dream`、`realityStatus=imagined`、`factuality=imagined` 的当日工作记忆，并写入稳定的 `eventKey=dream:<localDate>` 与事件指纹。旧 Dream 只与 Dream 合并或转存，不能与事实工作记忆混合；人格证据构造会排除所有 Dream，并要求真实 `eventKey` 与真实会话或上下文键，不能用记录 ID、事件类型或来源字段代替独立事件和场景。每条人格证据的影响分数还必须不低于 0.65，且至少三条真实、互不重复的独立事件覆盖至少两个上下文和至少 14 天，距离上次成功微调至少 30 天；每晚最多修改一个目标文件，只接受最多 80 字的一条温和倾向，并用内容安全门与 revision CAS 防止越界修改或覆盖管理员并发编辑。
