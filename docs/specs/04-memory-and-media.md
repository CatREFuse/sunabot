...
管理台可为所选 Agent 手动触发当日 Dream。触发继续受 Agent、系统时区自然日唯一约束：当日失败记录可由管理员明确恢复，当日已完成或有效租约仍在时拒绝建立第二份结果。历史显示工作记忆压缩数、长期记忆新增数、梦境正文和尝试状态，不显示原因、内部推理、旧的合并、归档、转存、人格或场域知识结果。
