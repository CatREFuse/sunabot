...
Provider 请求使用文本响应格式，提示词中的 JSON 只描述首选形状。旧持久 Dream 模板先通过 `dream-flex-contract-v3` 保留式迁移只替换 `response_format`，再由 `dream-memory-contract-v5` 精确替换官方 v3 或 v4 记忆合同；管理员自定义前后文、消息顺序与工具保持不变，无法精确识别的自定义合同不改写。重复、未知、遗漏或交叉复用 ID 不触发完整分区失败；宿主按可执行来源安全降级，并把原始生成内容随 Dream 输出持久化，供后续格式清洗与审计读取。
