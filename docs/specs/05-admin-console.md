...
设置中的 Agent 工具页默认打开“工具目录”Tab，列出十六个真实工具的图标、名称、Function 名、摘要、配置状态、运行能力和同步或异步方式，支持搜索、启停与刷新。详情弹层展示实际模型描述、描述来源、JSON Schema 参数与严格模式；`no_reply` 详情同时显示“no_reply 时戳一戳”，`system_config` 显示为内置同步工具。编辑模型描述会建立全局覆盖，“恢复继承说明”会删除覆盖。“运行参数”Tab 继续管理单轮调用上限、Tavily、Codex Worker 和图像生成默认值。两个 Tab 共用当前工具配置草稿和保存栏。

Agent 工具页只控制当前 Agent 的工具总开关。会话详情和 Web Chat 页提供“工具”入口，以弹层读取并保存当前会话的工具选择；Agent 已停用的工具在会话弹层中锁定关闭，运行能力暂不可用的工具显示当前状态但保留会话选择。弹层支持点击外部、Escape 和关闭按钮收起，保存只提交严格的 `disabledTools` 数组。`GET/PUT /api/conversations/:id/tools` 同时支持 QQ 会话 ID 与 `web:admin`，未知工具、重复项、多余字段和无效会话 ID 返回 400。
