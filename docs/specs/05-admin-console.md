...
Agent 工具页只控制当前 Agent 的工具总开关。每条会话列表项、会话详情页和 Web Chat 页均提供“会话设置”入口，进入可深链、可刷新、可返回原会话的独立设置页；QQ 会话包含“回复”和“工具权限”两个分区，Web Chat 只显示“工具权限”。回复分区编辑当前会话的回复开关，真实群聊同时编辑群聊编排器开关；工具权限分区读取并保存当前会话的工具选择。Agent 已停用的工具在会话设置页锁定关闭，运行能力暂不可用的工具显示当前状态但保留会话选择。每个分区使用独立草稿与保存栏，离开页面前保护未保存修改；工具目录必须在当前 Agent 与完整会话 ID 的数据全部就绪后才能编辑和保存，工具保存只提交严格的 `disabledTools` 数组。`GET/PUT /api/conversations/:id/tools` 同时支持 QQ 会话 ID 与 `web:admin`，未知工具、重复项、多余字段和无效会话 ID 返回 400。
