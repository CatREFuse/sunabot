...
会话详情把“启动”和群聊“编排器”作为消息区上方的快捷开关；编排器在回复关闭时保留显示并禁用。Token 统计默认只显示会话累计总消耗 widget，完整模型、行为、记忆与消息统计点击后在右侧栏展开，不常驻占用消息空间。会话设置也使用右侧栏，设置栏与 Token 详情栏的关闭按钮都位于左上角；侧栏支持遮罩关闭、Escape、焦点恢复和移动端满宽适配。
