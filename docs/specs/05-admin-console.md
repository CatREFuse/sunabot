...
设置中的 Agent 工具页默认打开“工具目录”Tab，列出十六个真实工具的图标、名称、Function 名、摘要、配置状态和同步或异步方式，支持搜索、启停与刷新。健康的运行能力不显示状态标记；仅当运行能力异常时，列表行和详情弹层显示“能力异常”及 API 返回的具体原因。详情弹层还展示实际模型描述、描述来源、JSON Schema 参数与严格模式；`no_reply` 详情同时显示“no_reply 时戳一戳”，`system_config` 显示为内置同步工具。编辑模型描述会建立全局覆盖，“恢复继承说明”会删除覆盖。“运行参数”Tab 继续管理单轮调用上限、Tavily、Codex Worker 和图像生成默认值。两个 Tab 共用当前工具配置草稿并自动串行同步。

Agent 工具页只控制当前 Agent 的工具总开关。每条会话列表项、会话详情页和 Web Chat 页均提供“会话设置”入口，进入可深链、可刷新、可返回原会话的独立设置页；QQ 会话包含“回复”和“工具权限”两个分区，Web Chat 只显示“工具权限”。回复分区编辑当前会话的回复开关，真实群聊同时编辑群聊编排器开关；工具权限分区读取并同步当前会话的工具选择。Agent 已停用的工具在会话设置页锁定关闭；健康的运行能力不显示状态标记，运行异常时显示 API 返回的具体原因并保留会话选择。`activate_skill` 与 `read_skill_resource` 的能力状态只反映运行端口是否接线，空 Skill 目录不显示异常。回复与工具权限共用当前页面实例的防抖串行队列，分区内就地显示等待、同步、成功或失败状态，不显示保存、放弃、未保存标记或离页确认；离开页面先刷新队列，失败时保留输入并留在当前页。工具目录必须在当前 Agent 与完整会话 ID 的数据全部就绪后才能编辑；会话或 Agent 切换取消旧上下文请求，工具同步只提交严格的 `disabledTools` 数组。`GET/PUT /api/conversations/:id/tools` 同时支持 QQ 会话 ID 与 `web:admin`，未知工具、重复项、多余字段和无效会话 ID 返回 400。
...
会话详情把“启动”和群聊“编排器”作为消息区上方的快捷开关；编排器在回复关闭时保留显示并禁用。Token 统计默认只显示会话累计总消耗 widget，完整模型、行为、记忆与消息统计点击后在右侧栏展开，不常驻占用消息空间。会话设置也使用右侧栏，包含回复控制、会话信息和工具权限；回复与工具开关点击后立即同步并生效，工具权限继续受 Agent 总开关约束。设置栏与 Token 详情栏的关闭按钮都位于左上角；侧栏支持遮罩关闭、Escape、焦点恢复和移动端满宽适配。
