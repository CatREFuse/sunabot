...
`read_file` 与 `write_file` 的 Function Call 批次和跨轮工具独占门禁已经由共享执行器覆盖；OpenAI Responses、Chat Completions、Anthropic、Gemini 与 Codex Responses 对同响应普通 assistant 文本的发送前独占校验仍是集成阻断。五种协议完成统一门禁和回归前，文件工具不能作为可上线能力交付。
