...
灵魂文件导入是既有 Agent 人格提示词的独立批量事务。预览绑定上传字节 SHA-256 与目标全部 persona-scope 文件的聚合 revision；确认时任一摘要或 revision 变化返回冲突且零写入。服务层在进入批量事务前完成严格 schema、大小、UTF-8、`AGENT_FILE_DEFINITIONS` 映射和内容摘要校验；文件仓库在同一管理写互斥锁内完成全部提示词结构、变量、路径和 revision 预校验，再为全部目标文件创建 0600 暂存与备份，并在第一次 rename 前持久化、同步 prepared journal。全部文件提交且 runtime reload 成功后才持久化 committed 终态并清理事务证据；进程退出或掉电后，API 在创建任何 Agent Runtime 前恢复 prepared 事务或验证 committed 文件并完成清理。提交或 reload 失败时全量恢复；journal、备份或恢复校验失败时进入统一 `CONFIG_RECOVERY_REQUIRED` 状态。来源 metadata 不修改目标 Agent ID、名称、manifest、账号或业务数据。
...
- `workspace/secrets/admin-credentials.json`：首次 `up` 的命令行 Landing 写入管理员名称与 scrypt 密码记录；密码和确认值只从交互式终端读取且不回显，文件为 0600，不进入 Git；
...
- `workspace/business/agents/<agentId>/agent.json`：Agent 名称、启用状态、系统提示词覆盖开关、Bot 行为、工具覆盖、Native Bash 对抗审批设置与 OneBot 行为配置；旧 Bash 路由值只作为一次性迁移输入，不能恢复第二个工具或改变平台固定权限：Linux/WSL 的 OneBot 会话使用 Native Bubblewrap，macOS 只允许管理员私聊宿主 Bash；
...
- `workspace/business/agents/<agentId>/workbench/knowledge/`：当前 Agent 唯一的人工知识资料树；管理 API 只以 `agentId` 选择目标，运行时递归读取 JSONL、Markdown 与文本普通文件，并把文件清单原子发布为同目录 `index.json`。资料是用户维护的源文件，不是会话或系统增长日志，不能跨 Agent 读取，也不进入 Git；
- `workspace/cache/knowledge/<agentId>.sqlite`：从当前 Agent 唯一资料树重建的 FTS5/BM25 索引缓存，不属于业务主库、queue 或离线恢复点；删除缓存后必须能从资料树完整重建；
- `workspace/business/agents/<agentId>/workbench/selfie/references.jsonl`：当前 Agent 唯一的 schemaVersion 1 自拍素材清单，每行严格保存一项 `{schemaVersion,id,fileName,note}`，最多 9 项；`id` 是图片内容 SHA-256，`note` 必填并限制为 1—120 个 Unicode code point，空文件或单个结尾换行表示空图库。清单使用同目录 0600 临时文件原子替换，拒绝额外字段、重复 ID、未知 schema、非法 Unicode、控制字符、超量内容及符号链接；图片仍是单张最多 8 MiB 的普通文件，清单属于小型可审阅配置，不替代 SQLite 承载增长型业务数据；
...
- `0.1.2` 到 `0.1.3` 通过 `npm run upgrade:0.1.3 -- plan|apply` 完成；没有 SQLite schema 或资源布局迁移，`apply` 仍固定停服并创建全 Agent 双库恢复点，再启动全部 Agent 运行保留式提示词迁移，最后执行 status 与 doctor。plan 必须零写入；提示词首次迁移由 registry 创建一次 0600 备份和 journal，已完成 marker 后不回填管理员修改；当前代码继续把已知旧合同保留式迁移到 `conversation-chat-media-v5`；
- `0.2.0` 到 `0.3.0` 通过 `npm run upgrade:0.3.0 -- plan|apply|verify|rollback` 完成。plan 对全部 Agent 逐文件预检；内容冲突、类型冲突、链接、特殊文件或非空旧投影会生成冲突报告并在资源与 SQLite 零修改状态失败。apply 只合并目标缺失或字节相同的普通文件，创建全 Agent SQLite 与两套迁移前资源恢复点，校验 SQLite 摘要不变，再把旧 `docker-workbench/` 归档进恢复点；verify 检查唯一 Workbench、固定入口、归档和摘要，rollback 在无迁移后 drift 时恢复两套原目录；
...
- `workspace/business/agents/<agentId>/docker-workbench/`：只允许作为 `0.2.0` 升级输入存在；`0.3.0` 迁移成功后整棵目录进入该次恢复点的 `archived/agents/<agentId>/docker-workbench/`，运行时、管理 API 与管理台不再创建、读取或展示该根；
- `workspace/business/agents/<agentId>/workbench/skills/` 与 `extensions/mcp/`：Native Bash、MCP 与 Skill Script 通过受控投影读取对应配置；Linux/WSL Bubblewrap 内 canonical Workbench 映射为 `/workbench`，Skill 与 MCP 控制文件保持只读；
...
- Bash backend 不再是可修改偏好；单调配置 epoch、审批结果、独立 Provider 实例、abort signal、审批票据与 capability 快照不持久化。管理员 QQ 私聊与已认证管理员 Web Chat可取得 `native_bash`，其他会话没有 Bash capability；每条命令使用独立对抗审批合同，票据固定绑定 Native backend。macOS 使用受审批的宿主 Bash，Linux/WSL 使用 Bubblewrap；两者均不访问 Docker socket。缺字段、过期、重放或绑定不一致均拒绝；
- Provider 可执行 Bash options 只能由当前真实 OneBot 入站或已认证管理员 Web Chat 即时构造，必须在同一不可变配置快照中包含 epoch、Native backend、canonical Workbench、access mode、strict mode、宿主权威 `isAdmin`、原始 `userRequest`、独立 audit runner、`isCurrent` 和完整审批上下文。Workbench 的规范路径、父链和目录身份必须在审计前冻结，并在隔离 probe 与最终 spawn 前复验；旧 handle 以 `BASH_CONFIGURATION_STALE` 失败关闭且不得产生审批、探针或执行副作用。API catalog、模型返回参数和持久配置都不能单独升格为执行权限；
- `sunabot-skill` 只作为 Native/admin Bash handle 内当前 Agent 的受管仓库端口存在，不写入 PATH、不向其他会话投影。install 只接受 canonical Workbench 相对 `.zip`，拒绝路径穿越、符号链接、多硬链接、特殊文件、空文件和超过 16 MiB 的文件；打开时使用 `O_NOFOLLOW`，读取前后复验 dev、ino、nlink、size、mtime 与 ctime，再把内存 Buffer 交给既有 Skill ZIP 门禁。review、enable 与 status 只接受合法 Skill ID，review 必须带 `--approve`，任何审计非 allow、配置过期或 Agent 绑定不完整都在仓库调用前失败关闭；
- `workspace/business/media/`：保存普通生成图片、持久附件和 `conversation-assets/agents/<agentId>/<sha256>.<ext>` 会话图片归档；归档只保存已经作为 workbench 参考快照、成功 `send_file` 图片读取或本轮实际命中并通过完整性校验的出站表情字节，assistant 会话消息仅持久化 `/generated-images/conversation-assets/...` URL，不保存宿主路径、Base64 或可变 workbench 定位。出站表情只有远端发送成功后才把已准备的归档 URL 写入 assistant 会话媒体；发送失败留下的未引用内容寻址文件不形成历史句柄。各 Agent 的唯一表情清单与引用 PNG/GIF 位于 `workbench/emoji/`；完整业务恢复、跨机迁移或远程搬迁必须复制并核对该目录，缺少会话归档会使对应历史媒体句柄不可解析，缺少清单或任一引用文件时该项不能进入可用图库；
...
Skill 脚本执行生成独立的一次性 Native 投影，不直接挂载 Agent extensions 源目录。投影先通过 descriptor 读取和完整包摘要复验复制当前 Skill，再写入只含 Skill ID、包摘要和目标脚本 manifest 的非秘密绑定文件；投影父目录为当前 UID 独占的 0700 私有目录，投影发布后目录为 0500、文件为 0400。root、workbench、skills、script 与 manifest 在投影完成后冻结 canonical path、dev、ino、ctime、uid、mode 和逐级路径身份，并在 spawn 前完整复验。沙箱不读取 MCP 描述符或 credential vault，不继承宿主、代理或 MCP 环境变量；Linux/WSL Bubblewrap invocation 只暴露 `/skills` RO、`/workbench` RO、临时 `/tmp` RW、固定虚拟环境和无网络 namespace。执行能力不得回退到 Docker，隔离或审计不完整时隐藏脚本 capability。

每个 stdio MCP server 启动前生成当前 Agent、当前 server 专属的 digest-pinned Native 投影：已批准 Skills 复制到只读 `/skills`，只含该 server 非秘密描述符的配置以只读方式挂载到 `/run/sunabot/extensions/mcp.json`，当前 Agent `workbench` 是唯一可写持久挂载。投影不包含 Agent workspace 根、HOME、其他 server、代理变量、Docker Env 或 Docker socket；server secret resolver 将 Agent、server 与描述符逻辑 key 派生为唯一宿主环境变量名，只从该物理名称读取值，再以逻辑 key 注入目标进程，禁止回退读取同名全局环境变量。生产 stdio 默认关闭；启用时只接受 Linux/WSL Bubblewrap 后端和绝对、root 所有、单硬链接、`0444` 的批准清单，运行时不得下载依赖。Linux/WSL 使用 bwrap、prlimit、独立 PID/用户/网络/cgroup namespace 和进程组；运行时网络固定关闭，macOS 缺少等价强隔离时失败关闭。进程退出、启动探针失败、禁用、Agent 关闭和 SIGKILL 清理必须先把同一目录 inode 原子移入已绑定父目录内的随机 quarantine，再覆盖并截断秘密文件；wipe 失败必须保留该 quarantine、返回稳定清理错误并允许按原 dev/ino 重试，不能以递归删除成功替代 wipe 成功。
...
`0.1.0`—`0.3.0` 老实例升级到当前 revision 时必须按目标版本逐级执行版本专用脚本；每个脚本只允许在对应目标版本的完整代码包中运行，不能跳过中间版本或通过修改版本文件绕过 `TARGET_RELEASE_MISMATCH`。达到 `0.2.0` 后必须保持停服执行 `upgrade:0.3.0`，完成全 Agent 恢复点、逐文件无冲突合并、旧根归档与单一 Workbench 验证，再启动当前 schema 的前向迁移。完整路线、表情、自拍参考图与知识库验收和回滚边界见 `docs/migrations/upgrade-old-versions-to-current.md`。
...
业务 schema v15 的既有 Dream 表继续前向兼容。`dream_runs` 保存自然日唯一运行、系统时区、自动或手动窗口、输入摘要、三字段模型输出、租约、重试与添加结果；旧 `memory_recall_stats`、`memory_recall_receipts` 与 `dream_memory_archive` 表暂不删除，当前 Dream 管线不读取、初始化、更新、归档或清除其中数据，长期记忆遗忘功能后续单独设计。

同一 Agent、系统时区自然日的 Dream run 唯一，claim、generated、consolidated 与 completed 阶段继续受租约和条件更新保护；persona 阶段固定记为 `none`，不执行人格读写。管理员对 completed 行发起显式手动触发时，以 `status='completed'` 条件更新原子重开该行：写入当前计划时间、窗口、seed、digest 与输入，清空生成、整理、人格、错误和终态字段，把状态设为 `running` 并将 `attempt_count` 重置为 1。条件更新未命中或有效租约存在时不得覆盖当前运行。模型输出先持久化为 generated，重启恢复直接继续提交；暂时模型或传输故障最多尝试三次、间隔 15 分钟，永久输入、长期记忆快照、结果冲突以及 Provider 明确返回的不可重试 HTTP 4xx 只尝试一次。工作记忆 CAS 与长期记忆 add-only 替换快照共享现有恢复和回滚边界，事务必须证明最终长期记忆集合包含提交前全部记录且只增加本次去重后的新记录。

调度表达式固定为系统 IANA 时区的 `0 4 * * *`，按当地自然日唯一并由时区库处理夏令时。Core 在 04:00 后启动时只补最近一次未运行日期，不遍历历史欠账；全新安装在首次 04:00 之前不会补做安装前一天。运行时每分钟检查一次，stop 会取消当前模型请求并释放定时器；完成记录不会被自动调度重复执行，但允许管理员显式重开。
...
- WebFetch 静态抓取接受带主机名的 HTTP(S) URL。Linux/WSL 动态 Renderer 使用 Lightpanda 0.3.3，不依赖 Chrome、Chromium 或 Playwright；受控代理支持带鉴权的 HTTP、HTTPS 与 CONNECT，并执行单响应、累计响应、请求数、原始字节、并发、队列、取消和总时限边界。renderer `/render` 只接受启动器生成的 bearer token，不接收 query，不挂载 workspace、secrets、Provider key、Codex 授权、OneBot/NapCat 凭据或浏览器用户目录，不保存 Cookie，并阻断下载、WebSocket、非 GET 及不受支持资源。Linux/WSL 使用随包 Bubblewrap、独立临时 HOME/cache/run，并遮蔽仓库、workspace 与凭据。Lightpanda、Bubblewrap、平台隔离或健康值无法验证时动态能力失败关闭，静态抓取保持可用；macOS 动态能力固定 unavailable。清理后的正文只进入 5 分钟进程内 LRU，不新增持久化格式或原始 HTML 日志。
