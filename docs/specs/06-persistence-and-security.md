...
- `0.1.2` 到 `0.1.3` 通过 `npm run upgrade:0.1.3 -- plan|apply` 完成；没有 SQLite schema 或资源布局迁移，`apply` 仍固定停服并创建全 Agent 双库恢复点，再启动全部 Agent 运行 `conversation-chat-media-v1` 保留式提示词迁移，最后执行 status 与 doctor。plan 必须零写入；提示词首次迁移由 registry 创建一次 0600 备份和 journal，已完成 marker 后不回填管理员修改；
