...
- `sunabot-skill` 只作为 Native/admin Bash handle 内当前 Agent 的受管仓库端口存在，不写入 PATH、不向 Docker 或其他会话投影。install 只接受 Native workbench 相对 `.zip`，拒绝路径穿越、符号链接、多硬链接、特殊文件、空文件和超过 16 MiB 的文件；打开时使用 `O_NOFOLLOW`，读取前后复验 dev、ino、nlink、size、mtime 与 ctime，再把内存 Buffer 交给既有 Skill ZIP 门禁。review、enable 与 status 只接受合法 Skill ID，review 必须带 `--approve`，任何审计非 allow、配置过期或 Agent 绑定不完整都在仓库调用前失败关闭；
...
- WebFetch 静态抓取接受任意带主机名的 HTTP(S) URL，静态请求和每次重定向均直接交给常规网络解析与连接，不按 DNS 结果、IP 范围、端口或 Fake-IP 筛除地址。动态 renderer 当前只允许 HTTP 导航与 HTTP 子请求，HTTPS 在入口失败关闭且代理拒绝 CONNECT；子请求同样不做地址来源筛选，同时受单响应、累计响应、请求数、并发、队列和取消边界约束。响应解压后和 renderer DOM 各自受 4 MiB 上限。renderer `/render` 只接受启动器生成的 bearer token，不接收 query，不挂载 workspace、secrets、Provider key、Codex 授权、OneBot/NapCat 凭据或浏览器用户目录，不保存 Cookie，并阻断下载、WebSocket、非 GET、压缩代理响应及图片、字体、音视频资源。macOS Native Core 使用无 workspace/secret mount 的独立 Docker renderer；Linux/WSL Native renderer 使用 Bubblewrap、独立临时 HOME/cache/run，并遮蔽仓库、workspace、凭据和浏览器用户目录。Chromium sandbox、平台隔离或健康值无法验证时动态能力失败关闭，静态抓取保持可用。清理后的正文只进入 5 分钟进程内 LRU，不新增持久化格式或原始 HTML 日志。
