...
- `workspace/business/agents/<agentId>/docker-workbench/`：当前 Agent 的隔离 Bash 独立可写持久目录，不与文件工具共享；同样以 write-if-missing 的 `index.md` 作为查询入口。运行时把完整 Native `workbench/` 只读投影到该容器的 `/workbench/native-workbench/`；
...
- Codex JSONL：`exec --json` 与 `app-server --stdio` 子进程通信协议；普通 deferred worker 的隔离 Codex home 仍位于任务目录，管理员控制模式的本机线程保存在主 Codex home、远端线程保存在目标主机 Codex home。Sunabot SQLite 只保存异步 job 参数、终态、回调和运行中进程身份，不复制或另建 Codex 会话历史。
