...
| Agent 设置 | 记忆与编排 | 记忆处理、群聊编排 | 记忆模型、阈值、容量、编排模型与窗口 |
| Agent 设置 | 工具 | Agent 工具、命令执行 | 八项 Agent 工具目录、运行参数、联网、Codex Worker、图像与隔离命令 |
| 系统设置 | 公共系统 | 模型服务、广播风暴、账户安全、连接与通知 | Provider、广播风暴嗅探与 m/n/k 参数、管理员密码、Bark 监控、OneBot WebSocket 路径与 Access Token 状态 |
...
| 系统提示词 | 公共对话、记忆、编排、群聊总结和自拍改写提示词 | 无 |
