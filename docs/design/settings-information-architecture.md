...
Agent 工具开关是当前 Agent 的上限，会话只能在该上限内进一步停用。Agent 已停用的工具在会话页保持关闭并锁定；健康能力不显示状态，能力异常时显示具体原因并保留当前会话的选择。回复与工具权限使用互不混合的草稿和保存栏，离开页面前确认未保存修改；工具目录未完成当前 Agent 与完整会话 ID 的读取时保持锁定，切换分区、会话或 Agent 时不得显示、保存或带入上一上下文的工具选择。
