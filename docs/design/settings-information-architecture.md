...
Agent 工具页分为“工具目录”和“运行参数”两个 Tab。工具目录固定展示全部 Agent 工具；运行参数保留单轮调用上限、Tavily、Codex Worker 和图像默认值。`send_file` 展示配置状态，实时 OneBot 能力健康时不额外标记，异常时显示具体原因，不提供账号或目标参数；`send_voice_message` 当前固定显示能力异常，不能通过配置启用。

| 区域 | 内容 | 交互 |
| --- | --- | --- |
| 工具目录 | 图标、名称、Function 名、摘要、执行方式、启用状态 | 行内切换启停；健康能力无标记，能力异常时显示原因并保留开关；点击详情查看参数和完整说明 |
...
工具启用与运行能力使用不同状态。用户配置决定启用或停用，平台、会话权限和依赖决定运行门禁；健康能力不显示状态标记，只有能力异常时使用 Boxicons、文字和语义色共同提示。说明覆盖、启用覆盖与运行参数在“Agent 工具”分区自动串行同步，切换 Tab 不丢失正在同步或失败后保留的输入。
...
Agent 工具开关是当前 Agent 的上限，会话只能在该上限内进一步停用。Agent 已停用的工具在会话页保持关闭并锁定；健康能力不显示状态，能力异常时显示具体原因并保留当前会话的选择。回复与工具权限使用互不混合的草稿，并共用当前页面实例的防抖串行同步队列；每个分区只显示等待、同步、成功或失败状态，不显示保存、放弃、未保存标记或离页确认。离开页面先刷新队列，失败时保留当前输入并留页；工具目录未完成当前 Agent 与完整会话 ID 的读取时保持锁定，会话或 Agent 切换时取消旧上下文请求，不得显示、同步或带入上一上下文的工具选择。
